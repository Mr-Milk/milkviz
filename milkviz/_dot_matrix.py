...
from typing import Optional, List, Union, Tuple, Any, Dict, Literal
...
class DotHeatmap:
    ...
    def __init__(
        ...
        if dot_patch == "circle":
            ...
            self.ax.scatter(self.xcoord, self.ycoord, s=circ_size,
                            c=circ_colors)
            self.add_dot_size_legend()
        else:
            norm_sizes = norm_arr(dot_size, size=(0, 1))
            for rx, ry, ratio, c in zip(self.xcoord, self.ycoord, norm_sizes,
                                        circ_colors):
                if not np.isnan(ratio):
                    pie = mpatches.Wedge((rx, ry), 0.4, 90, 360 * ratio + 90,
        ...
        if matrix_hue is not None:
            ...
            rects = PatchCollection(rects, array=rects_colors,
                                    cmap=self.matrix_cmap, alpha=alpha)
    ...
    def _set_grid(self):
        ...
        self.ax.set_xticks(np.arange(X), labels=self.xticklabels, rotation=90,
                           ha="center")
    ...
    def add_dot_size_legend(self):
        ...
        self.dot_size_legend = SizeLegend(sizes=self.sizes,
                                          ax=self.ax,
                                          array=self.dot_size,
                                          dtype=self.dtype,
                                          **options
